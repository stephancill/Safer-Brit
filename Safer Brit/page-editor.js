...
try {
	if (window.location.href.indexOf("britannica.com") > -1) {
...
window.onload = function () {
	try {
		document.getElementsByClassName("google-ad-container")[0].style.display = "none"
	} catch(err) {
		console.log(err)
	}
...
function handleText(textNode) {
	...
	// Todo: fix this
	words.map(word => {
		textNode.nodeValue = textNode.nodeValue.replace(new RegExp('\\b' + word + '\\b', 'gi'), "!@#$%")
	})
}
